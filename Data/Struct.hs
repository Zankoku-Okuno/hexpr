...
{-| Template from which a costruct may be created.
    Generally, you would define a Costruct type, then transform the type into a
    template for values, and use that template to create actual values.
-}
...
{-| Create an 'Struct' identical to the input, but with no fields filled .-}
